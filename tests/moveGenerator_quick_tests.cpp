...
/*    std::string allpawnmoves_fen = "pkp5/PP3p2/p1p5/PP6/pp6/P1P5/pp3P2/PKP5 w - - 0 1";*/
...
TEST (moveGenerator_tests, attacks_test)
{
    chess::engine::Engine engine ("Test engine", "Test author");
    std::string position = "8/8/8/8/1p6/2p2p2/8/PQKNBR1p w - - 0 1";
    engine.Position (position);
    chess::consts::bitboard attacks = engine.GetAttacks (true);
    int expected_results = 2 + 5 + 6 + 3 + 1 + 1 + 1 + 0;
    int result = chess::bitboard_helper::count (attacks);
    std::cout << position << std::endl;
    std::cout << chess::bitboard_helper::visualize_bitboard (attacks) << std::endl;
    EXPECT_EQ (expected_results, result);

    position = "pqknbr1P/8/2P2P2/1P6/8/8/8/8 b - - 0 1";
    engine.Position (position);
    attacks = engine.GetAttacks (false);
    expected_results = 2 + 5 + 6 + 3 + 1 + 1 + 1 + 0;
    std::cout << position << std::endl;
    std::cout << chess::bitboard_helper::visualize_bitboard (attacks) << std::endl;

    EXPECT_EQ (expected_results, chess::bitboard_helper::count (attacks));

    position = "8/8/8/8/1p6/2p2p2/8/PQKNBR1p w - - 0 1";
    engine.Position (position);
    attacks = engine.GetAttacks (chess::consts::Q, chess::consts::Square::C4);
    expected_results = 1 + 2 + 4 + 4 + 5 + 2 + 1 + 2;
    std::cout << position << std::endl;
    std::cout << chess::bitboard_helper::visualize_bitboard (attacks) << std::endl;
    EXPECT_EQ (expected_results, chess::bitboard_helper::count (attacks));
}
